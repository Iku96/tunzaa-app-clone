...
export default function AddProductScreen() {
    ...
    React.useEffect(() => {
        if (user) {
            console.log('👤 [AddProduct] User Profile Role:', user.activeProfileRole);
            console.log('🏢 [AddProduct] Vendor Profile Found:', !!vendorProfile);
            if (vendorProfile) {
                console.log('📦 [AddProduct] Metadata Keys:', Object.keys(vendorProfile.metadata || {}));
                console.log('🔑 [AddProduct] Derived VendorID:', vendorId);
                console.log('🏪 [AddProduct] Derived StoreID:', storeId);
            }
        }
    }, [user, vendorProfile, vendorId, storeId]);
    ...
    React.useEffect(() => {
        if (categoriesData) {
            console.log('📂 [AddProduct] Categories loaded:', categoriesData.items?.length || 0);
        }
        if (categoriesError) {
            console.error('❌ [AddProduct] Categories fetch failed:', categoriesError);
        }
                    ...
                    <View style={styles.modalContent}>
                        ...
                        {categoriesLoading ? (
                            ...
                            <View style={{ padding: 40, alignItems: 'center' }}>
                                ...
                                <TouchableOpacity 
                                    onPress={() => router.replace('/(vendor)/index')}
                                    style={{ marginTop: 20, padding: 10, backgroundColor: '#F3F4F6', borderRadius: 8 }}
                                >
                                    <Text style={{ color: '#3A5BA9' }}>Refresh Dashboard</Text>
                                </TouchableOpacity>
                            </View>
                        )}
                    </View>
