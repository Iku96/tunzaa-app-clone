...
// Define delivery onboarding steps
const STEPS = [
    { title: 'Intro', path: '/mauzo-intro' },
    { title: 'Shop Details', path: '/(delivery)/onboarding/step-1' },
    { title: 'Location', path: '/(delivery)/onboarding/step-2' },
    { title: 'Review', path: '/(delivery)/onboarding/step-3' },
    { title: 'Documents', path: '/(delivery)/onboarding/step-4' },
];
...
export default function DeliveryOnboardingLayout() {
    ...
    // Get the current file name (e.g., 'step-1' or 'index')
    const currentRouteName = segments[segments.length - 1] as string;

    const getActiveStepIndex = () => {
        if (currentRouteName === 'step-1' || currentRouteName === 'index') return 1;
