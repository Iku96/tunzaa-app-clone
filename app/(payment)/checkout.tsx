...
const CheckoutScreen = () => {
  ...
  const handleCartRestoration = useCallback(async () => {
    ...
    if (tempCart?.cart) {
      try {
        ...
        setError(t('payment.failed_restore_cart'));
      } finally {
  ...
  // Reset state only on initial mount, not on every focus
  // This prevents clearing user selections when modals open/close
  ...
  // Handle cart restoration on unmount
  useFocusEffect(
    useCallback(() => {
      return () => {
        // Restore cart on unmount if needed
        const tempCart = cart.getTempCart();
        if (tempCart?.cart) {
  ...
  const isStepCompleted = (step: CheckoutStep): boolean => {
    switch (step) {
      ...
      case CheckoutStep.ORDER_SUMMARY:
        // If payments disabled, auto-select cash_on_delivery
  ...
  const handleContinueToPayment = async () => {
    ...
    if (isDeliveryEnabled) {
      if (!selectedAddressId || !selectedDeliveryType) {
        setError(t("payment.please_complete_checkout"));
        return;
      }
      ...
      if (!buyerProfile?.delivery_address || !Array.isArray(buyerProfile.delivery_address)) {
        setError(t("payment.please_select_address"));
        return;
      }
    ...
    // Validate payment only if payments enabled
    ...
    return (
      <SafeAreaView className="flex-1 bg-background" edges={["top", "right", "left"]}>
        ...
        <View className="flex-1 justify-center items-center">
          <Text className="text-muted-foreground">{t("payment.loading_cart")}</Text>
        </View>
      </SafeAreaView>
    );
  }
          ...
          <View className="flex-1 justify-center items-center p-4">
            <Alert icon={Terminal} variant="default" className="mb-4">
              <Text className="text-sm text-foreground">
                {t("payment.cart_empty_connection")}
              </Text>
            </Alert>
            <View className="flex-row space-x-2">
              <Button variant="outline" onPress={() => cart.refetch()}>
                <Text>{t("payment.refresh_cart")}</Text>
              </Button>
              <Button onPress={handleBackNavigation}>
                <Text>{t("payment.go_back")}</Text>
              </Button>
            </View>
          </View>
        </SafeAreaView>
          ...
          <View className="flex-1 justify-center items-center p-4">
            <Alert icon={Terminal} variant="destructive" className="mb-4">
              <Text className="text-sm text-destructive">
                {t("payment.invalid_cart_data")}
              </Text>
            </Alert>
            <Button onPress={() => router.push("/(buyer)/cart")}>
              <Text>{t("payment.go_to_cart")}</Text>
            </Button>
          </View>
        </SafeAreaView>
      );
  ...
  return (
    ...
    >
      <SafeAreaView className="flex-1 bg-background">
        <View className={`${isDesktop ? 'w-[80%] max-w-7xl mx-auto bg-white' : 'flex-1 w-100% bg-background'} `}>
          {!isDesktop && (
                    ...
                    <View ref={orderOverviewRef}>
                      <Card className="m-4">
                        <CheckoutStepHeader
                          ...
                          title={t("payment.order_overview")}
                          icon={ShoppingBag}
                        ...
                        {(!cart.hasItems() && hasCheckoutProgress) && (
                          <View className="p-4 border-t border-border bg-yellow-50">
                            <Text className="text-sm text-yellow-800 text-center">
                              {t("payment.cart_empty_processing")}
                            </Text>
                          </View>
                        )}
                      </Card>
                    ...
                    <View ref={deliveryAddressRef}>
                      <Card className={`m-4 ${!isStepAccessible(CheckoutStep.DELIVERY_ADDRESS) ? 'opacity-50' : ''}`}>
                        <CheckoutStepHeader
                          ...
                          title={t("payment.delivery_address")}
                          icon={MapPin}
                        ...
                        {isStepAccessible(CheckoutStep.DELIVERY_ADDRESS) ? (
                          ...
                          <View className="p-4 border-t border-border">
                            <Text className="text-sm text-muted-foreground text-center">
                              {t("payment.complete_overview_first")}
                            </Text>
                          </View>
                        )}
                      </Card>
                    ...
                    <View ref={deliveryOptionsRef}>
                      <Card className={`m-4 ${!isStepAccessible(CheckoutStep.DELIVERY_OPTIONS) ? 'opacity-50' : ''}`}>
                        <CheckoutStepHeader
                          ...
                          title={t("payment.delivery_options")}
                          icon={Truck}
                        ...
                        {isStepAccessible(CheckoutStep.DELIVERY_OPTIONS) ? (
                            ...
                            {isUsingFallback && (
                              <View className="p-3 bg-yellow-50 border-b border-yellow-200">
                                <Text className="text-xs text-yellow-800 text-center">
                                  {t("payment.using_default_delivery")}
                                </Text>
                              </View>
                            )}
                            <DeliveryOptionsSection
                          ...
                          <View className="p-4 border-t border-border">
                            <Text className="text-sm text-muted-foreground text-center">
                              {t("payment.select_address_first")}
                            </Text>
                          </View>
                        )}
                      </Card>
                    ...
                    <View ref={orderSummaryRef}>
                      <Card className={`m-4 ${!isStepAccessible(CheckoutStep.ORDER_SUMMARY) ? 'opacity-50' : ''}`}>
                        ...
                        {isStepAccessible(CheckoutStep.ORDER_SUMMARY) ? (
                          ...
                          <View className="p-4 border-t border-border">
                            <Text className="text-sm text-muted-foreground text-center">
                              {isDeliveryEnabled ? t("payment.select_delivery_first") : t("payment.complete_overview_first")}
                            </Text>
                          </View>
                        )}
                      </Card>
                    ...
                    <View ref={similarItemsRef} className="m-4">
                      <View className="px-4 max-w-7xl mx-auto">
                        <SimilarItems
                          ...
                          count={6}
                          title={t("recommendations.you_may_also_like")}
                        />
                      </View>
                    </View>
                  </View>
