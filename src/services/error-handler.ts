...
export function parseApiError(error: any): ErrorResponse {
    ...
    const isRatings404 = status === 404 && url.includes('/ratings/entities/');
    if (!isRatings404) {
        console.log(`❌ [API Error] ${method.toUpperCase()} ${url} returned ${status}`);
        if (errorData) console.log(`❌ [API Error Data]`, JSON.stringify(errorData));
    }
    ...
    if (matchedError) {
        // Prioritize server message if it exists, even if we found a match in errors.json
        ...
        return {
            ...
            message: (typeof serverMessage === 'string' && serverMessage) ? serverMessage : matchedError.message,
