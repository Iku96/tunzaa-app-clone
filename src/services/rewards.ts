...
export const rewardsApi = {
  ...
  getUserBalance: async (params: GetBalanceParams = {}): Promise<UserBalance> => {
    ...
    if (params.include_history !== undefined) {
      queryParams.append("include_history", params.include_history.toString());
    }
    ...
    const response = await apiClient.get<UserBalance>(
      `/rewards/balance?${queryParams.toString()}`
    );
    return response.data;
  },
...
export const rewardsUtils = {
  ...
  calculateCouponValue: (points: number, redemptionRate: number = 100): number => {
    return (points / redemptionRate) * 500; // 500 TZS base value
  },
