...
export const useAddToWishlist = () => {
    ...
    return useMutation({
        ...
        onSuccess: (data) => {
            ...
            // Proactively update the Zustand store
            useWishlistStore.getState().addItem(data);
        },
        onError: (error: any) => {
...
export const useRemoveFromWishlist = () => {
    ...
    return useMutation({
        ...
        onSuccess: (_, { productId, variantSku }) => {
            queryClient.invalidateQueries({ queryKey: ["wishlist"] });
            queryClient.invalidateQueries({ queryKey: ["wishlist-count"] });
            queryClient.invalidateQueries({ queryKey: ["wishlist-status"] });
            
            // Proactively remove from Zustand store
            useWishlistStore.getState().removeItem(productId, variantSku);
        },
    });
};
