...
export default function AddProductModal({ visible, onClose, onSuccess }: AddProductModalProps) {
    ...
    const vendorId = vendorProfile?.metadata?.vendor_id || vendorProfile?.profile_id || vendorProfile?.vendor_id;
    const storeId = vendorProfile?.metadata?.stores?.[0]?.store_id || vendorProfile?.metadata?.store_id || vendorProfile?.profile_id || vendorProfile?.store_id;

    useEffect(() => {
        if (visible && user) {
            console.log('👤 [AddProductModal] User Profile Role:', user.activeProfileRole);
            console.log('🏢 [AddProductModal] Vendor Profile Found:', !!vendorProfile);
            if (vendorProfile) {
                console.log('📦 [AddProductModal] Metadata Keys:', Object.keys(vendorProfile.metadata || {}));
                console.log('🔑 [AddProductModal] Derived VendorID:', vendorId);
                console.log('🏪 [AddProductModal] Derived StoreID:', storeId);
            }
        }
    }, [visible, user, vendorProfile, vendorId, storeId]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            for (let i = 0; i < localImages.length; i++) {
                ...
                const uploadResult = await uploadApi.uploadFile(uri, filename) as any;
                const finalUrl = uploadResult.url || uploadResult.fileCDNUrl || uploadResult.file_url || uploadResult.data?.url;
                ...
                if (finalUrl) {
                    ...
                    console.error('❌ [AddProduct] Upload succeeded but no URL found in response:', uploadResult);
                }
            }
